...
async function cleanup() {
  ...
  const tables = [
    ...
    'organizacao_relacionamentos', 'organizacoes',
    'plano_pagamentos', 'plano_assinaturas', 'planos_saas', 'apoios_plataforma',
  ...
  for (let i = 0; i < terreirosData.length; i++) {
    ...
    const terreiro = await prisma.terreiros.create({
      data: {
        ...
        taxonomyCategory: TaxonomyCategory.COMUNIDADE,
  ...
  for (const t of terreiros) {
    const e = await prisma.eventos.create({
      data: {
        ...
        tipo: EventType.GIRA,
        taxonomyCategory: TaxonomyCategory.EVENTO,
...
async function createInstituicoes() {
  ...
  for (const i of data) {
    await prisma.instituicoes.create({
      data: { ...i, criadoPorId: admin.id, taxonomyCategory: TaxonomyCategory.INSTITUICAO },
    });
  }
...
async function createHubContent() {
  ...
  for (let i = 0; i < terreiros.length; i++) {
    ...
    await prisma.conteudos.create({
      data: { ...c, publicado: true, criadoPorId: admin.id, terreiroId: t.id, taxonomyCategory: TaxonomyCategory.CONTEUDO },
    });
  }
...
async function createRegioes() {
  console.log('🌍 Criando taxonomia geográfica (regiões)...');

  const ids: Record<string, string> = {};

  const upsertRegiao = async (
    slug: string,
    nome: string,
    tipo: RegiaoTipo,
    ordenacao: number,
  ...
  for (const [i, c] of continentes.entries()) {
    await upsertRegiao(c.slug, c.nome, RegiaoTipo.CONTINENTE, i + 1);
    totalRegioes++;
  }

  for (const [i, p] of paises.entries()) {
    await upsertRegiao(p.slug, p.nome, RegiaoTipo.PAIS, i + 1, p.continente);
    totalRegioes++;
  }

  for (const [i, r] of regioesBrasil.entries()) {
    await upsertRegiao(r.slug, r.nome, RegiaoTipo.REGIAO, i + 1, 'brasil');
    totalRegioes++;

    for (const [j, t] of r.territorios.entries()) {
      await upsertRegiao(t.slug, t.nome, RegiaoTipo.TERRITORIO, j + 1, r.slug);
      totalRegioes++;
  ...
  console.log(`  ✓ ${totalRegioes} regiões criadas`);
  console.log(`  ✓ ${totalTradicoes} relações de tradição criadas`);
}
